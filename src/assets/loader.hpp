...
private:
  struct response_t {
    ...
    response_t(asset_bundle& bundle_, F&& callback_, std::string_view err_) :
        callback{std::forward<F>(callback_)}, data{std::in_place_type_t<std::string_view>{}, err_},
        bundle{&bundle_} {}
...
public:
  expect<asset_bundle::rmodel_idx> load_rmodel(asset_bundle& bundle, const std::string& path,
                                               const std::string& name, const model_opts& opts);
