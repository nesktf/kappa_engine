...
std::pair<const void*, u32> rigged_model_data::vertex_data(u32 attr_idx, u32 mesh_idx) const {
  enum {
    ATTR_POS = 0,
    ATTR_NORM,
    ATTR_UVS,
...
model3d_rig_asset::model3d_rig_asset(
    shogle::shader_storage_buffer&& ssbo, ntf::unique_array<bone_t>&& bones,
    std::unordered_map<std::string_view, u32>&& bone_reg, ntf::unique_array<mat4>&& bone_locals,
    ntf::unique_array<mat4>&& bone_inv_models, ntf::unique_array<mat4>&& bone_transforms,
    ntf::unique_array<mat4>&& transf_cache, ntf::unique_array<mat4>&& transf_output) noexcept :
    _ssbo{std::move(ssbo)},
    _bones{std::move(bones)}, _bone_reg{std::move(bone_reg)}, _bone_locals{std::move(bone_locals)},
    _bone_inv_models{std::move(bone_inv_models)}, _bone_transforms{std::move(bone_transforms)},
    _transf_cache{std::move(transf_cache)}, _transf_output{std::move(transf_output)} {}

expect<model3d_rig_asset> model3d_rig_asset::create(const model_rig_data& rigs,
  ...
  try {
    ...
    auto ssbo = shogle::shader_storage_buffer::create(
        ctx, {
...
void model3d_rig_asset::tick_bones(const mat4& root) {
...
bool model3d_rig_asset::set_transform(std::string_view bone, const bone_transform& transf) {
...
bool model3d_rig_asset::set_transform(std::string_view bone, const mat4& transf) {
  auto it = _bone_reg.find(bone);
  if (it == _bone_reg.end()) {
    return false;
  }
...
void model3d_rig_asset::set_transform(u32 bone, const mat4& transf) {
  NTF_ASSERT(bone < _bone_transforms.size());
  _bone_transforms[bone] = transf;
}

void model3d_rig_asset::set_transform(u32 bone, const bone_transform& transf) {
  NTF_ASSERT(bone < _bone_transforms.size());
  constexpr vec3 pivot{0.f, 0.f, 0.f};
  _bone_transforms[bone] = shogle::build_trs_matrix(transf.pos, transf.scale, pivot, transf.rot);
}

void model3d_rig_asset::set_transform(u32 bone, shogle::transform3d<f32>& transf) {
  NTF_ASSERT(bone < _bone_transforms.size());
  _bone_transforms[bone] = transf.local();
}

ntf::optional<u32> model3d_rig_asset::find_bone(std::string_view name) {
...
bool model3d_rig_asset::set_transform(std::string_view bone, shogle::transform3d<f32>& transf) {
  return set_transform(bone, transf.local());
}

void model3d_rig_asset::apply_animation(const model_anim_data& anims, std::string_view name,
                                        u32 tick) {
...
u32 model3d_rig_asset::retrieve_buffer_bindings(std::vector<shogle::shader_binding>& binds) const {
...
rigged_model3d_asset::rigged_model3d_asset(model3d_mesh_buffers<rigged_model_data>&& meshes,
                                           model3d_mesh_textures&& texturer,
                                           model3d_rig_asset&& rigger,
    ...
    model3d_mesh_textures{std::move(texturer)}, model3d_rig_asset{std::move(rigger)},
    _mats{std::move(mats)}, _mat_reg{std::move(mat_reg)}, _pip{std::move(pip)},
    _mesh_mats{std::move(mesh_texs)}, _name{std::move(name)}, _transf{} {}

expect<rigged_model3d> rigged_model3d_asset::create(data_t&& data) {
  ...
  auto pip =
      r.make_pipeline(VERT_SHADER_RIGGED_MODEL, FRAG_SHADER_RAW_ALBEDO, att_binds, pip_opts);
  if (!pip) {
  ...
  auto rigger = model3d_rig_asset::create(data.rigs, data.armature);
  if (!rigger) {
...
void rigged_model3d_asset::tick() {
  model_rigger::tick_bones(_transf.local());
}

u32 rigged_model3d_asset::retrieve_render_data(const scene_render_data& scene,
  ...
  for (u32 mesh_idx = 0; mesh_idx < mesh_count; ++mesh_idx) {
    ...
    const u32 rigger_bind_count = model3d_rig_asset::retrieve_buffer_bindings(data.bindings);
