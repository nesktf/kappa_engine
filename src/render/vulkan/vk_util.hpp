...
#define KA_VK_STRUCT(_typename, _sType)                                  \
  template<>                                                             \
  struct VkStructTraits<_typename> {                                     \
...
constexpr fn vkmk_zero(void* pNext = nullptr) -> T {
  T info{};
  info.sType = VkStructTraits<T>::sType;
  info.pNext = pNext;
  return info;
}
...
public:
  ...
  struct HandleSet {
    VkHandle handle;
    VkHandle parent;
    VkHandle other_parent;
  };

  struct DelData {
    DelData(HandleType type, VkHandle handle, VkHandle parent, VkHandle other_parent) :
...
public:
  ...
  fn enqueue_handle(VkHandle handle, VkHandle parent, HandleType type,
  ...
  fn enqueue(VkImageView image_view, VkDevice device) -> void {
    enqueue_handle((VkHandle)image_view, (VkHandle)device, TYPE_IMAGE_VIEW);
  }

  fn enqueue(VkSurfaceKHR surface, VkInstance instance) -> void {
    enqueue_handle((VkHandle)surface, (VkHandle)instance, TYPE_SURFACE);
  }

  fn enqueue(VkCommandPool pool, VkDevice device) -> void {
    enqueue_handle((VkHandle)pool, (VkHandle)device, TYPE_CMDPOOL);
  }

  fn enqueue(VkFence fence, VkDevice device) -> void {
    enqueue_handle((VkHandle)fence, (VkHandle)device, TYPE_FENCE);
  }

  fn enqueue(VkSemaphore semaphore, VkDevice device) -> void {
    enqueue_handle((VkHandle)semaphore, (VkHandle)device, TYPE_SEMAPHORE);
  }

  fn enqueue(VkDebugUtilsMessengerEXT messenger, VkInstance instance) -> void {
    enqueue_handle((VkHandle)messenger, (VkHandle)instance, TYPE_MESSENGER);
  }

  fn enqueue(VkDevice device) -> void {
    enqueue_handle((VkHandle)device, VK_NULL_HANDLE, TYPE_DEVICE);
  }

  fn enqueue(VkInstance instance) -> void {
    enqueue_handle((VkHandle)instance, VK_NULL_HANDLE, TYPE_INSTANCE);
  }

  fn enqueue(VkDescriptorPool pool, VkDevice device) -> void {
    enqueue_handle((VkHandle)pool, (VkHandle)device, TYPE_DESCPOOL);
  }

  fn enqueue(VkDescriptorSetLayout layout, VkDevice device) -> void {
    enqueue_handle((VkHandle)layout, (VkHandle)device, TYPE_DESCLAYOUT);
  }

  fn enqueue(VkPipelineLayout layout, VkDevice device) -> void {
    enqueue_handle((VkHandle)layout, (VkHandle)device, TYPE_PIPLAYOUT);
  }

  fn enqueue(VkPipeline pipeline, VkDevice device) -> void {
    enqueue_handle((VkHandle)pipeline, (VkHandle)device, TYPE_PIPELINE);
  }

  fn enqueue(VkShaderModule shader, VkDevice device) -> void {
    enqueue_handle((VkHandle)shader, (VkHandle)device, TYPE_SHADER);
  }
