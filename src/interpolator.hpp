...
struct easing_pow : public easing_mixer_base<T, U, easing_pow<T, U>> {
  ...
  constexpr easing_pow(u32 p) : _p{p} {}
...
public:
  constexpr lerper_base(const T& first, const T& last) : F{}, _first{first}, _last{last} {}
  ...
  constexpr lerper_base(const T& first, const T& last, std::in_place_t, Args&&... args) :
